...
class manager {
     ...
     */
    public function send_message($message, $userid = null, $params = array('parse_mode' => 'Markdown')) {
    ...
    public function config_form($preferences, $userid) {
        ...
        if (!$this->is_chatid_set($userid, $preferences)) {
            ...
            $url = new \moodle_url($this->redirect_uri(), [
                'action' => 'removechatid', 'userid' => $userid,
                'sesskey' => sesskey()
            ]);
     ...
     * @param string $key The name of the field that contains the submitted webhhok.
     * @return void
     */
    public function set_webhook($key) {
        // Get submitted data.
        $data = data_submitted();
        $webhook = $data->{$key};

        if (empty($this->config('sitebottoken'))) {
            return \core\notification::error(get_string('sitebottokennotsetup', 'message_telegram'));
        }
        ...
        if (empty($webhook) || $webhook === '') {
            ...
            if (!empty($response) && isset($response->error_code) && isset($response->description)) {
                return \core\notification::error($response->description);
            }
            return \core\notification::success(get_string('webhookdeleted', 'message_telegram'));
        }

        if (strpos($webhook, 'https:') !== 0) {
            // If submitted webhook is invalid reset config to previous configuration.
            $this->set_config('webhook', $this->config('webhook'));
            return \core\notification::error(get_string('requirehttps', 'message_telegram'));
        }

        $response = $this->send_api_command('setWebhook', ['url' => $webhook, 'allowed_updates' => 'message']);
        if (!empty($response) && isset($response->error_code) && isset($response->description)) {
            return \core\notification::error($response->description);
        }
